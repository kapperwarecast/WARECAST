-- ============================================================================
-- MIGRATION: Intégration échanges automatiques dans rent_or_access_movie
-- Date: 2025-11-16
-- Description: Le RPC devient le point d'entrée unique pour :
--              - Lecture de films possédés (ownership)
--              - Échange automatique + lecture de films non possédés
-- ============================================================================
...
BEGIN
  -- ========================================================================
  -- ÉTAPE 1: Vérifier si l'utilisateur possède déjà le film
  -- ========================================================================
  SELECT
    fr.id,
    fr.current_owner_id,
  ...
  -- Si le film n'est pas dans le registre (pas encore déposé)
  ...
  -- ========================================================================
  -- ÉTAPE 2: Si user possède le film → Session de lecture directe (gratuite)
  -- ========================================================================
  IF v_user_owns_film THEN
    -- Vérifier si user a déjà une session en cours sur CE film
    ...
    -- Créer nouvelle session de lecture (gratuite car propriétaire)
  ...
  -- ========================================================================
  -- ÉTAPE 3: User ne possède PAS le film → Vérifier disponibilité
  -- ========================================================================

  -- Vérifier si le propriétaire actuel a une session active
  SELECT NOT EXISTS(
  ...
  IF NOT v_film_available THEN
    RETURN json_build_object(
      'success', false,
      'error', 'Le film est actuellement en cours de lecture par son propriétaire',
      'code', 'FILM_NOT_AVAILABLE'
    );
  END IF;

  -- ========================================================================
  -- ÉTAPE 4: Trouver automatiquement un film disponible de l'utilisateur
  -- ========================================================================

  SELECT fr.id INTO v_user_film_to_exchange_id
  FROM films_registry fr
  WHERE fr.current_owner_id = p_auth_user_id
    AND fr.id != v_registry_id  -- Pas le même film
    AND NOT EXISTS(
  ...
  IF v_user_film_to_exchange_id IS NULL THEN
    RETURN json_build_object(
      'success', false,
      'error', 'Vous n''avez aucun film disponible à échanger (tous en cours de lecture)',
  ...
  -- Vérifier si paiement fourni et succeeded
  ...
  ELSE
    ...
    SELECT EXISTS(
      ...
      WHERE user_id = p_auth_user_id
        AND (statut = 'actif' OR (statut = 'résilié' AND date_expiration > NOW()))
    ) INTO v_user_has_subscription;

    IF NOT v_user_has_subscription THEN
      -- Non-abonné et pas de paiement → requires_payment
      RETURN json_build_object(
        'success', false,
  ...
  -- ========================================================================
  -- ÉTAPE 6: Libérer ancienne session si abonné (rotation)
  -- ========================================================================
  ...
  -- ========================================================================
  -- ÉTAPE 7: ÉCHANGE AUTOMATIQUE INSTANTANÉ
  -- ========================================================================

  -- Transfert bilatéral de propriété
  -- Film demandé : propriétaire actuel → utilisateur
  ...
  -- Film offert : utilisateur → propriétaire du film demandé
  UPDATE films_registry
  SET
    previous_owner_id = current_owner_id,
  ...
  -- Créer entrée d'échange dans l'historique
  ...
  -- Mettre à jour l'historique de propriété avec l'ID d'échange
  ...
  -- ========================================================================
  -- ÉTAPE 8: Créer session de lecture sur le nouveau film possédé
  -- ========================================================================
  ...
  RETURN json_build_object(
    ...
    'owns_film', true  -- Maintenant user possède le film
  );
...
COMMENT ON FUNCTION rent_or_access_movie IS 'Point d''entrée unique pour lecture de films : gère propriété, échange automatique, et sessions de lecture (48h)';

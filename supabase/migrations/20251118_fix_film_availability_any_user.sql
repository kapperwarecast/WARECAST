-- ============================================================================
-- MIGRATION: Fix film availability check - Check if ANYONE is watching
-- Date: 2025-11-18
-- Description: Fix ÉTAPE 3 to check if ANYONE (not just owner) has an active
--              session on the film, making it unavailable for exchange
-- Bug: Film showed as available when owner was watching it
-- Fix: Remove user_id filter to check all active sessions on the film
-- ============================================================================
...
BEGIN
  ...
  -- =========================================================================
  -- ÉTAPE 3: USER NE POSSÈDE PAS → Vérifier disponibilité (FIX: N'IMPORTE QUI)
  -- =========================================================================

  -- FIX: Vérifier si N'IMPORTE QUI regarde ce film
  SELECT NOT EXISTS(
    SELECT 1 FROM emprunts
    WHERE movie_id = p_movie_id  -- ← FIX: Plus de filtre user_id
      AND statut = 'en_cours'
      AND date_retour > NOW()
  ...
  IF NOT v_film_available THEN
    RETURN json_build_object(
      'success', false,
      'error', 'Film actuellement indisponible (en cours de lecture)',
...
-- Commentaire
COMMENT ON FUNCTION rent_or_access_movie IS 'RPC pour créer sessions et gérer échanges instantanés - Fix availability check for ANY user';

-- ============================================================================
-- MIGRATION: Fix film availability check in rent_or_access_movie
-- Date: 2025-11-17
-- Description: Corriger la vérification de disponibilité pour vérifier
--              CE FILM SPÉCIFIQUE et non pas n'importe quel film du propriétaire
-- BUG: L'échange était bloqué si le propriétaire regardait UN film quelconque
-- FIX: Vérifier uniquement si le propriétaire regarde LE FILM DEMANDÉ
-- ============================================================================
...
BEGIN
  ...
  SELECT NOT EXISTS(
    ...
    WHERE user_id = v_film_owner_id
      AND movie_id = p_movie_id  -- ← FIX: Ajouter cette condition
      AND statut = 'en_cours'
      AND date_retour > NOW()
...
-- Commentaire
COMMENT ON FUNCTION rent_or_access_movie IS 'RPC pour créer sessions et gérer échanges instantanés - Fix disponibilité film spécifique';

-- ============================================================================
-- MIGRATION: Use trigger for ownership_history instead of manual INSERT
-- Date: 2025-11-24
-- Description: Résout le conflit entre trigger automatique et INSERT manuels
--              en utilisant le trigger existant record_ownership_transfer()
--              Solution : Mettre à jour transfer_date dans les UPDATE de films_registry
-- ============================================================================

-- Drop old function
DROP FUNCTION IF EXISTS rent_or_access_movie(UUID, UUID, UUID, UUID);

-- Create with trigger-compatible approach
...
BEGIN
  ...
  ELSIF p_payment_id IS NOT NULL THEN
    SELECT COALESCE(
      (SELECT status = 'succeeded' FROM payments WHERE id = p_payment_id),
      FALSE
  ...
  -- ✅ FIX: Mettre à jour transfer_date + acquisition_method pour que le trigger fonctionne
  ...
  -- ⚠️ PLUS BESOIN D'INSERT MANUEL: Le trigger record_ownership_transfer() s'en occupe automatiquement
  -- Il insère dans ownership_history avec :
  -- - film_registry_id = NEW.id
  -- - from_owner_id = OLD.current_owner_id
  -- - to_owner_id = NEW.current_owner_id
  -- - transfer_type = NEW.acquisition_method ('exchange')
  -- - transfer_date = NEW.transfer_date (NOW())
...
FIX: Utilise trigger record_ownership_transfer() pour ownership_history (pas INSERT manuel).';

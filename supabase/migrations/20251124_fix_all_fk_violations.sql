-- ============================================================================
-- MIGRATION: Fix ALL FK violations in rent_or_access_movie
-- Date: 2025-11-24
-- Description:
--   ANALYSE COMPLÈTE DES SCHÉMAS - Correction finale de tous les FK
--   Problème ROOT: Confusion entre movie_id (movies.id) et registry_id (films_registry.id)
--
--   film_exchanges FK constraints:
--     - film_offered_id → films_registry(id)  [PAS movies(id)]
--     - film_requested_id → films_registry(id)  [PAS movies(id)]
--
--   Fix: Utiliser registry IDs partout dans film_exchanges
-- ============================================================================

-- Drop old function
DROP FUNCTION IF EXISTS rent_or_access_movie(UUID, UUID, UUID, UUID);

-- Create corrected version with ALL FK violations fixed
...
FIX FINAL COMPLET: Tous les FK corrects - film_exchanges utilise registry IDs (pas movie IDs).';

-- ============================================================================
-- MIGRATION: Fix film_exchanges column names
-- Date: 2025-11-24
-- Description: Corrige les noms de colonnes dans l'INSERT film_exchanges
--              Utilise film_offered_id, film_requested_id
--              (pas initiator_film_id, recipient_film_id)
-- ============================================================================

-- Drop old function
DROP FUNCTION IF EXISTS rent_or_access_movie(UUID, UUID, UUID, UUID);

-- Create with correct film_exchanges column names
...
BEGIN
  ...
  -- Mettre à jour transfer_date + acquisition_method pour que le trigger fonctionne
  ...
  -- ✅ FIX: Utilisation des BONS noms de colonnes dans film_exchanges
  INSERT INTO film_exchanges (
    initiator_id,
    recipient_id,
    film_offered_id,      -- ✅ Correct (pas initiator_film_id)
    film_requested_id,    -- ✅ Correct (pas recipient_film_id)
    status,
    completed_at
  )
  VALUES (
    p_auth_user_id,
    v_film_owner_id,
    v_offered_film_id,    -- Film offert PAR l'initiateur
    p_movie_id,           -- Film demandé (requested)
    'completed',
    NOW()
  )
...
FIX final: Noms de colonnes corrects dans film_exchanges (film_offered_id, film_requested_id).';

-- ============================================================================
-- MIGRATION: Simplification rent_or_access_movie (utilise get_movie_access_info)
-- Date: 2025-11-24
-- Description: Version simplifiée avec:
--   - Réutilisation de get_movie_access_info (supprime duplication)
--   - Gestion d'erreurs complète (bloc EXCEPTION)
--   - Initialisation correcte de v_payment_valid
--   - Structure simplifiée (moins de branches)
--   - Messages d'erreur détaillés (SQLSTATE + hints)
-- ============================================================================
...
DECLARE
  ...
  v_payment_valid BOOLEAN := FALSE; -- ✅ INITIALISÉ par défaut
BEGIN
  ...
  ELSIF p_payment_id IS NOT NULL THEN
    -- ✅ FIX: Gérer cas NULL explicitement avec COALESCE
    SELECT COALESCE(
...
-- ============================================================================
-- BLOC EXCEPTION GLOBAL (NOUVEAU)
-- Capture toutes les erreurs SQL et retourne JSON détaillé
-- ============================================================================
EXCEPTION
  WHEN OTHERS THEN
    RETURN json_build_object(
...
Version simplifiée avec gestion d''erreurs complète (EXCEPTION + SQLSTATE).';

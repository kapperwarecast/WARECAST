...
import { useWatchedRentals } from '@/hooks/data/use-watched-rentals'
    ...
    return (
      <main className="min-h-screen bg-black pt-24 pb-12 px-6">
        <div className="max-w-7xl mx-auto">
          {/* En-tête */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-white mb-2">Déjà visionné</h1>
            <p className="text-zinc-400">Films que vous avez déjà visionnés</p>
          </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-zinc-400">
            {pagination && !loading ? `${movies.length} film${movies.length > 1 ? 's' : ''} déjà visionné${movies.length > 1 ? 's' : ''}` : 'Films que vous avez déjà visionnés'}
          </p>
        </div>

        {/* Message si aucun film visionné */}
        {!loading && movies.length === 0 && (
          <div className="flex items-center justify-center min-h-[400px]">
            <div className="text-center">
              <div className="mb-4">
                <Check className="w-16 h-16 text-zinc-600 mx-auto mb-4" />
              </div>
              ...
              <div className="text-center mt-6 text-zinc-400 text-sm">
                ...
                {!pagination.hasNextPage && movies.length > 20 && (
                  <div className="mt-2 text-zinc-500">
                    ✓ Tous vos films visionnés ont été chargés
                  </div>
                )}
              </div>
            )}
